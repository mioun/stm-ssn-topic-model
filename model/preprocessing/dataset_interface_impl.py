...
from model.utils.file_uitis import save_data_file, load_data_file


class DatasetInterfaceImpl(DatasetInterface):
    ...
    def preprocess_data_set(self, docs_training, docs_test, train_labels, test_labels, categories):
        ...
        self._doc_tokens_training = self.tokenizer.tokenize(docs_training)
        self.feature_selection()
        self._doc_tokens_training, self._train_labels = self.reduce_tokens(self._doc_tokens_training,
                                                                           self._train_labels)
        doc_tokens_test = self.tokenizer.tokenize(docs_test)
        self._doc_tokens_test, self._test_labels = self.reduce_tokens(doc_tokens_test, self._test_labels)
