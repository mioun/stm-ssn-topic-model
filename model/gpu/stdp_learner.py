...
def stdp_linear_single_step(
        ...
        f_pre: Callable = lambda x: x, f_post: Callable = lambda x: x, sn=None
):
    ...
    # inhibition N_N_OUT
    # inhibition_mask = out_spike.any(dim=1, keepdim=True).expand_as(out_spike)
    # sn.mem[inhibition_mask] = sn.mem[inhibition_mask] - 0.5
    ...
    def step(self, on_grad: bool = True, scale: float = 1.):
        ...
        for _ in range(length):
            ...
            self.trace_pre, self.trace_post, dw = stdp_f(
                ...
                self.f_pre, self.f_post, sn=self.sn
            )
            if scale != 1.:
                dw *= scale
